  ...
  if (!window.location.pathname.includes("/blog/") || window.location.pathname === "/blog" || window.location.pathname === "/blog/") {
    ...
    if (!listWrappers.length) {
      ...
      listWrappers.forEach((wrapper, index) => {
        ...
        console.log(`📝 Other articles available: ${otherArticles.length}`);
        for (let i = otherArticles.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [otherArticles[i], otherArticles[j]] = [otherArticles[j], otherArticles[i]];
        }
        const itemsToShow = Math.min(targetCount, otherArticles.length);
        const relatedItems = otherArticles.slice(0, itemsToShow);
        console.log(`📊 Selected ${relatedItems.length} articles from ${otherArticles.length} available`);
        relatedItems.forEach((item, idx) => {
          const originalIndex = items.indexOf(item);
          console.log(`   ✅ Selected item ${originalIndex} (${idx + 1}/${relatedItems.length})`);
        });
